...
//
        ...
        int[][][] train_X_minibatch = new int[train_minibatch_N][minibatchSize]
                [nIn];
        double[][][] validation_X_minibatch = new
                double[validation_minibatch_N][minibatchSize][nIn];
        int[][][] validation_T_minibatch = new int[validation_minibatch_N]
                [minibatchSize][nOut];
...
//
        for (int pattern = 0; pattern < patterns; pattern++) {
            for (int n = 0; n < train_N_each; n++) {
                ...
                for (int i = 0; i < nIn; i++) {
                    if ((n_ >= train_N_each * pattern && n_ < train_N_each *
                            (pattern + 1)) &&
            ...
            for (int n = 0; n < validation_N_each; n++) {
                ...
                for (int i = 0; i < nIn; i++) {
                    if ((n_ >= validation_N_each * pattern && n_ <
                            validation_N_each * (pattern + 1)) &&
                            (i >= nIn_each * pattern && i < nIn_each * (pattern + 1))) {
                        validation_X[n_][i] = (double) binomial(1, 1 - pNoise_Training,
                                rng);
                    } else {
                        validation_X[n_][i] = (double) binomial(1, pNoise_Training,
                                rng);
                    }
                }
            ...
            for (int n = 0; n < test_N_each; n++) {
                ...
                for (int i = 0; i < nIn; i++) {
                    if ((n_ >= test_N_each * pattern && n_ < test_N_each * (pattern
                            + 1)) &&
                            (i >= nIn_each * pattern && i < nIn_each * (pattern + 1))) {
...
// create minibatches
        for (int j = 0; j < minibatchSize; j++) {
            ...
            for (int i = 0; i < validation_minibatch_N; i++) {
                validation_X_minibatch[i][j] = validation_X[minibatchIndex.get(i *
                        minibatchSize + j)];
                validation_T_minibatch[i][j] = validation_T[minibatchIndex.get(i *
                        minibatchSize + j)];
            }
        }
//
...
// pre-training the model
        ...
        classifier.pretrain(train_X_minibatch, minibatchSize, train_minibatch_N,
                pretrainEpochs, pretrainLearningRate, k);
        System.out.println("done.");
        ...
        for (int epoch = 0; epoch < finetuneEpochs; epoch++) {
            for (int batch = 0; batch < validation_minibatch_N; batch++) {
                classifier.finetune(validation_X_minibatch[batch],
                        validation_T_minibatch[batch], minibatchSize, finetuneLearningRate);
            }
